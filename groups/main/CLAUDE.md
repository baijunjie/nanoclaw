...
**网址**: https://polymarbot.com

### 产品定位
PolymarBot 是 Polymarket 自动交易机器人托管平台，提供专业的托管服务。

### 核心功能
1. **无需编程** - 用简单参数创建策略，几分钟内部署
2. **24/7 全天候运行** - 机器人监控市场并自动执行交易，无需人工干预
3. **策略排行榜** - 浏览策略排行榜，复制顶级策略快速盈利
4. **实时监控** - 通过实时仪表板监控机器人执行状态、交易历史和仓位变化
5. **多钱包管理** - 管理多个钱包分散投资组合和风险
6. **无 Gas 交易** - 使用 Polymarket 的 Gnosis Safe 合约无 Gas 费交易

### 使用流程
1. **注册账户** - 几秒钟内创建账户
2. **添加钱包** - 创建 Polymarket 代理钱包（Gnosis Safe）
3. **创建策略** - 定义交易规则和参数
4. **部署机器人** - 启动机器人，自动交易

### 定价方案
| 方案 | 价格 | 钱包数 | 策略数 | 机器人数 | 单策略上限 | 统计访问 |
|------|------|--------|--------|----------|-----------|----------|
| Free | $0 | 5 | 5 | 5 | $2 | 无 |
| Pro | $19/月 | 20 | 20 | 20 | $20 | 部分 |
| Plus | $99/月 | 100 | 100 | 100 | $100 | 完整 |
| Max | $999/月 | 1,000 | 1,000 | 1,000 | 无限 | 完整 |
...
1. 使用 WebFetch 访问新闻页面提取图片 URL
2. 用 curl 下载图片到 /workspace/group/media/
3. 图片命名：polymarket_news_$(date +%s).jpg
4. 发推文时使用 image_paths 参数附带图片
5. 发布成功后用 rm 命令删除图片
